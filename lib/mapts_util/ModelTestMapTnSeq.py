...
def FindWorkingModel(fastq_fp, models_dir, cfg_d):
    """

    Args:
        fastq_fp (str): Path to FastQ file
        models_dir (str): Path to directory with model files
        cfg_d (dict): Contains keys listed in function ValidateConfig
    
    Returns:
        model_results_list (list<ret_d>):
            ret_d (d):
                ...
                start_frac (float): Number start/ Total reads
                end_frac (float): Number end/ Total reads
                model_name (str): Name of model
            

    Description:
        Go through each model in the models_dir,
        run the barcode finding part of the program
        against the fastq file 'fastq_fp' 
    """
    # Get all the model filepaths in a list.
    model_fps = [os.path.join(models_dir, x) for x in os.listdir(models_dir)]
    model_results_list = []
    for model_fp in model_fps:
        model_results = RunModelTestMapTnSeq(model_fp, fastq_fp, cfg_d)
        model_results["model_name"] = os.path.basename(model_fp)
        model_results_list.append(model_results)
        start_frac = model_results["nModelStartFound"]/model_results["nReads"]
        end_frac = model_results["nModelEndFound"]/model_results["nReads"]
        model_results["start_frac"] = start_frac
        model_results["end_frac"] = end_frac
...
def RunModelTestMapTnSeq(model_fp, fastq_fp, cfg_d):
    """
    Args:
        model_fp (str)
        fastq_fp (str)
        cfg_d (d): Args given in ValidateConfig
    Returns:
        ret_d:
            nReads (int): Number of reads gone through
            nModelStartFound (int): Number of reads with start of model found
    ...
    # The number of reads with barcodes found in them is:
    #nameToBarcode = ret_d["nameToBarcode"]
    #nBarcodesFound = len(nameToBarcode.keys())

    # If we are only testing for correctness of the model
    # we exit the program at this phase
    logging.info("Model: {} test phase passed.".format(model_fp))
...
def find_barcodes_and_end_of_transposon(fastq_fp, model_str, flanking,
    ...
    while (maxReads is None or (nReads < maxReads)):
        ...
        # within the sequence
        transposonEnd = FindModelEnd(seq, model_str, expModelStart,
                                     Find_cfg_d)

        if transposonEnd is not None:
            nModelEndFound += 1
        
        '''
        if (transposonEnd is not None) and (len(seq) >= transposonEnd + minScore):
    ...
    # All 'nX' variables are ints. mapnames is 
    ret_d = {
            "nReads": nReads,
            "nModelStartFound": nModelStartFound,
            "nModelEndFound": nModelEndFound
            }
